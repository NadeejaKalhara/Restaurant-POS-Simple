...
const categoryColors = {
  appetizers: 'bg-orange-500/10 text-orange-400',
  mains: 'bg-blue-500/10 text-blue-400',
  drinks: 'bg-cyan-500/10 text-cyan-400',
  desserts: 'bg-pink-500/10 text-pink-400',
};
...
export default function MenuManager({ menuItems }) {
  ...
  const handleEdit = (item) => {
    setEditingItem(item);
    setFormData({ name: item.name, price: item.price.toString(), category: item.category, available: item.available ?? true });
    setIsOpen(true);
  };
          ...
          <DialogContent className="bg-slate-800 border-slate-700">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button type="button" variant="outline" onClick={resetForm} className="flex-1 border-slate-600 text-slate-300">
                  Cancel
                </Button>
          ...
          <TableBody>
            {menuItems.map((item) => (
              <TableRow key={item.id} className="border-slate-700/50">
                ...
                <TableCell className="text-amber-400">${item.price.toFixed(2)}</TableCell>
                <TableCell>
                  <Badge variant={item.available !== false ? 'default' : 'secondary'} 
                    className={item.available !== false ? 'bg-green-500/10 text-green-400' : 'bg-slate-600 text-slate-400'}>
                ...
                <TableCell className="text-right">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(item.id)} className="text-slate-400 hover:text-red-400">
